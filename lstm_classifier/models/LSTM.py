...
class LSTMClassifier(nn.Module):
	...
	def forward(self, input_sentence, batch_size=None):
		...
		if batch_size is None:
			h_0 = Variable(torch.zeros(NUM_LAYERS, self.batch_size, self.hidden_size).cuda()) # Initial hidden state of the LSTM
			c_0 = Variable(torch.zeros(NUM_LAYERS, self.batch_size, self.hidden_size).cuda()) # Initial cell state of the LSTM
		else:
