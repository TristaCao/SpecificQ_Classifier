...
def polarity(q_tokens, liwc_dict, liwc_prefix):
    ...
    for token in q_tokens:
        ...
        if cat != set():
            ...
            if 'certain' in cat:
                certain_ct += 1
    return insight_ct, discrep_ct, certain_ct
...
#           filtered = [w for w in tokens if not w in stopwords.words('english')]
    ...
    with open("processed_data.csv") as file:
        ...
        for row in f:
            ...
            f16 = word_count(q_tokens, word_type).tolist()
            f5, f6, f7 = idf(q_tokens,idf_dic)
            f8, f9, f10 = polarity(q_tokens, liwc_dict, liwc_prefix)
            f11, f12, f13 = num_hypernyms(q_tokens)
            f14 = hypernym_match(q_tokens,c_tokens)
            f15 = similarity(q_tokens,c_tokens, boe)
